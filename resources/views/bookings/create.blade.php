...
<div class="max-w-xl mx-auto bg-white rounded-lg shadow-sm p-6 mt-6">
    ...
    <div class="flex justify-between items-start mb-4">
        <div>
            <h2 class="text-xl font-bold">ຈອງຫ້ອງປະຊຸມ</h2>
    ...
    <div class="bg-red-50 rounded-lg p-3 flex justify-between items-center mb-4">
        <div>
            ...
            <p class="text-xs text-gray-500">ຄວາມຈຸ: {{ $room->capacity }}</p>
        </div>
    ...
    <form method="POST" action="{{ route('bookings.store') }}" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="title" value="{{ old('title') }}" placeholder="ເຊັ່ນ ການວາງແຜນປະຈຳໄຕມາດ"
                   class="w-full border rounded-lg p-2 text-sm" required>
        </div>

        <div>
            <label class="text-xs font-medium text-gray-600">Description (Optional)</label>
            <textarea name="description" rows="3" placeholder="ລາຍລະອຽດວາລະການປະຊຸມໂດຍຫຍໍ້..."
        ...
        <div>
            ...
            <div class="flex items-center gap-3 mt-1">
                ...
                <span class="text-xs text-gray-400">ສູງສຸດ {{ $room->capacity }} ຄົນ (ຄວາມຈຸຫ້ອງ)</span>
            </div>
        </div>

        <div class="flex justify-end gap-3 pt-3 border-t">
            <a href="{{ route('rooms.show', $room) }}" class="border rounded-lg px-4 py-2 text-sm">ຍົກເລີກ</a>
            <button type="submit" class="bg-red-700 text-white rounded-lg px-4 py-2 text-sm">ຢືນຢັນການຈອງ</button>
        </div>
    </form>
</div>
