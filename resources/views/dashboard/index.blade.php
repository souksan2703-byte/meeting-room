                        ...
                        <div class="border-l p-1.5 space-y-1">
                            @foreach ($cellBookings as $booking)
                                <button type="button"
                                   ...
                                   data-attendees="{{ $booking->attendeesCount() }}"
                                   data-description="{{ $booking->description }}"
                                   class="block w-full text-left rounded px-1.5 py-1 text-[11px] leading-tight cursor-pointer
                                          {{ $booking->status === 'pending' ? 'bg-red-50 text-red-600 border border-red-200' : 'bg-red-700 text-white' }}">
                                    <span class="block font-semibold truncate">{{ $booking->start_time->format('H:i') }} - {{ $booking->end_time->format('H:i') }}</span>
                                    <span class="block truncate opacity-90">{{ $booking->title }}</span>
                                </button>
                            @endforeach
                        </div>
                    @endforeach
                        ...
                        @foreach ($dayBookings->take(3) as $booking)
                            <button type="button"
                               ...
                               class="block w-full text-left text-[10px] truncate rounded px-1 py-0.5 mb-0.5 cursor-pointer
                                ...
                                {{ $booking->start_time->format('H:i') }}-{{ $booking->end_time->format('H:i') }} {{ $booking->title }}
                            </button>
                        @endforeach
